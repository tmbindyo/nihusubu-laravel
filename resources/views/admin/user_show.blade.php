                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <div class="row">
                            <div class="col-sm-12">
                                <p>Edit.</p>
                                <form method="post" action="{{ route('admin.user.update',$institutionUser->id) }}" autocomplete="off" class="form-horizontal form-label-left">
                                    @csrf
                                    ...
                                    <div class="has-warning">
                                        ...
                                        @endif
                                        <input type="email" name="email" value="{{$institutionUser->email}}" class="form-control input-lg {{ $errors->has('email') ? ' is-invalid' : '' }}">
                                        <i>email</i>
                                    </div>
                                    <br>
                                    <div class="has-warning">
                        ...
                        @can('admin view roles')
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover dataTables-example" >
                                    ...
                                    <tbody>
                                    @foreach($institutionUser->adminRoles as $role)
                                        <tr class="gradeX">
                                            <td>{{$role->name}}</td>
                                            <td class="text-right">
                                                <div class="btn-group">
                                                    @can('admin assign role')
                                                        <a href="{{ route('admin.user.delist.role', ['id'=>encrypt($institutionUser->id), 'role_id'=>encrypt($role->id)]) }}" class="btn-danger btn btn-xs">Delete</a>
                                                    @endcan
                                                </div>
                                            </td>
                                        </tr>
...
@include('admin.layouts.modals.user_add_role')
